let dataTableInstance,passwordUpdateForm=document.getElementById("passwordForm"),addProductBtn=document.getElementById("addProductBtn"),addProductForm=document.getElementById("addProductForm"),editProductForm=document.getElementById("editProductForm");function adminFetchProducts(){fetch("/products").then(t=>t.json()).then(t=>{let a=document.getElementById("productsTableBody");a.innerHTML="",dataTableInstance&&dataTableInstance.clear(),t.data.forEach(t=>{var e=[t.name,t.description,t.category,t.price,t.quantity,t.retailer,`<button class="btn btn-primary" onclick="openEditModal('${t._id}')">Edit</button>
                    ...
                    `,a.appendChild(e))}),dataTableInstance?dataTableInstance.draw():dataTableInstance=new DataTable("#productsTable",{paging:!0,searching:!0,ordering:!0,info:!0,pageLength:8})}).catch(t=>console.error("Error loading products:",t))}function adminModal(t,e){var a=document.getElementById("addProductModal"),o=document.getElementById("editProductModal");M.Modal.init(a),M.Modal.init(o);let d=document.getElementById("addProductCategory"),n=document.getElementById("productCategory"),r=document.getElementById("addProductRetailer"),c=document.getElementById("productRetailer");t.forEach(t=>{var e=document.createElement("option");e.value=t,e.textContent=t,d.appendChild(e.cloneNode(!0)),n.appendChild(e.cloneNode(!0))}),e.forEach(t=>{var e=document.createElement("option");e.value=t,e.textContent=t,r.appendChild(e.cloneNode(!0)),c.appendChild(e.cloneNode(!0))}),M.FormSelect.init(d),M.FormSelect.init(n),M.FormSelect.init(r),M.FormSelect.init(c);a=document.querySelectorAll("select");M.FormSelect.init(a,{dropdownOptions:{constrainWidth:!0,container:document.body}})}function openEditModal(e){fetch("/products/"+e).then(t=>t.json()).then(t=>{document.getElementById("productName").value=t.data.name,document.getElementById("productDescription").value=t.data.description,document.getElementById("productCategory").value=t.data.category,document.getElementById("productPrice").value=t.data.price,document.getElementById("productQuantity").value=t.data.quantity,document.getElementById("productRetailer").value=t.data.retailer,document.getElementById("editProductForm").dataset.productId=e,M.updateTextFields(),M.Modal.getInstance(document.getElementById("editProductModal")).open()}).catch(t=>console.error("Error fetching product details:",t))}async function deleteProduct(t){if(confirm("Are you sure you want to delete this product?"))try{(await fetch(`/products/${t}/delete`,{method:"DELETE"})).ok?(showToast("Product deleted successfully"),adminFetchProducts()):showToast("Error deleting product")}catch(t){console.error("Error:",t),showToast("Error deleting product")}}function fetchOrders(){fetch("/admin/orders").then(t=>t.json()).then(t=>{let i=document.getElementById("ordersTableBody");i.innerHTML="",Array.isArray(t)?(t.forEach(t=>{var e=t.userId?`${t.shippingAddress.street}, ${t.shippingAddress.city}, `+t.shippingAddress.country:"N/A",a=t.userId&&t.userId._id?t.userId.fullName:"N/A",o=t.totalAmount,d=t.status,n=new Date(t.orderedAt).toLocaleDateString(),r=t.shippedAt?new Date(t.shippedAt).toLocaleDateString():"N/A",c=t.deliveredAt?new Date(t.deliveredAt).toLocaleDateString():"N/A";let s="";t.orderItems.forEach(t=>{var e=t.productId?t.productId.name:"Unknown Product",a=t.price,t=t.quantity;s+=`<p>${e} (x${t}) - $${a}</p>`});t=document.createElement("tr");t.innerHTML=`
                        <td>${a}</td>
                        <td>${e}</td>
                        <td>${s}</td>
                        <td>$${o}</td>
    ...
    `).join(""))}async function updateQuantity(t,e){e<1||(await fetch("/cart/update/"+t,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({quantity:e})})).ok&&(loadCart(),updateCartCount())}async function removeItem(t){(await fetch("/cart/delete/"+t,{method:"DELETE"})).ok&&(loadCart(),updateCartCount())}async function clearCart(){(await fetch("/cart/clear",{method:"DELETE"})).ok&&(loadCart(),updateCartCount())}function updateCartTotal(t){t=t.reduce((t,e)=>t+e.productId.price*e.quantity,0);document.getElementById("cartTotal").textContent=t.toFixed(2)}function checkout(){window.location.href="/checkout"}async function loadCheckoutItems(){try{var t=await(await fetch("/cart/items")).json();t.success&&(displayCheckoutItems(t.cartItems),updateTotalAmount(t.cartItems))}catch(t){M.toast({html:"Error loading cart items"})}}function displayCheckoutItems(t){document.getElementById("checkoutItems").innerHTML=t.map(t=>`
        ...
        `,a.appendChild(e)})):console.log("No products available to display.")}function showProductDetails(e){let a=productsList.find(t=>t._id===e);var t=document.getElementById("productName"),o=document.getElementById("productCategory"),d=document.getElementById("productPrice"),n=document.getElementById("productDesc"),r=document.getElementById("similarProductsContainer");let c=document.getElementById("productImageCarousel");a?(t.innerText=a.name,o.innerText=a.category,d.innerText=a.price,n.innerText=a.description,c.innerHTML="",a.images.forEach(t=>{var e=document.createElement("a");e.classList.add("carousel-item"),e.href="#",e.innerHTML=`<img src="${t}" alt="${a.name}">`,c.appendChild(e)}),setTimeout(()=>{M.Carousel.init(c,{fullWidth:!1,indicators:!0}).set(1)},100),t=productsList.filter(t=>t.category===a.category&&t._id!==e).slice(0,5),r.innerHTML=t.map(t=>`
