let dataTableInstance,passwordUpdateForm=document.getElementById("passwordForm"),addProductBtn=document.getElementById("addProductBtn"),addProductForm=document.getElementById("addProductForm"),editProductForm=document.getElementById("editProductForm");function adminFetchProducts(){fetch("/products").then(t=>t.json()).then(t=>{let a=document.getElementById("productsTableBody");a.innerHTML="",dataTableInstance&&dataTableInstance.clear(),t.data.forEach(t=>{var e=[t.name,t.description,t.category,t.price,t.quantity,t.retailer,`<button class="btn btn-primary" onclick="openEditModal('${t._id}')">Edit</button>
                    ...
                    `,a.appendChild(e))}),dataTableInstance?dataTableInstance.draw():dataTableInstance=new DataTable("#productsTable",{paging:!0,searching:!0,ordering:!0,info:!0,pageLength:8})}).catch(t=>console.error("Error loading products:",t))}function adminModal(t,e){var a=document.getElementById("addProductModal"),o=document.getElementById("editProductModal");M.Modal.init(a),M.Modal.init(o);let n=document.getElementById("addProductCategory"),d=document.getElementById("productCategory"),c=document.getElementById("addProductRetailer"),r=document.getElementById("productRetailer");t.forEach(t=>{var e=document.createElement("option");e.value=t,e.textContent=t,n.appendChild(e.cloneNode(!0)),d.appendChild(e.cloneNode(!0))}),e.forEach(t=>{var e=document.createElement("option");e.value=t,e.textContent=t,c.appendChild(e.cloneNode(!0)),r.appendChild(e.cloneNode(!0))}),M.FormSelect.init(n),M.FormSelect.init(d),M.FormSelect.init(c),M.FormSelect.init(r);a=document.querySelectorAll("select");M.FormSelect.init(a,{dropdownOptions:{constrainWidth:!0,container:document.body}})}function openEditModal(e){fetch("/products/"+e).then(t=>t.json()).then(t=>{document.getElementById("productName").value=t.data.name,document.getElementById("productDescription").value=t.data.description,document.getElementById("productCategory").value=t.data.category,document.getElementById("productPrice").value=t.data.price,document.getElementById("productQuantity").value=t.data.quantity,document.getElementById("productRetailer").value=t.data.retailer,document.getElementById("editProductForm").dataset.productId=e,M.updateTextFields(),M.Modal.getInstance(document.getElementById("editProductModal")).open()}).catch(t=>console.error("Error fetching product details:",t))}async function deleteProduct(t){if(confirm("Are you sure you want to delete this product?"))try{(await fetch(`/products/${t}/delete`,{method:"DELETE"})).ok?(showToast("Product deleted successfully"),fetchProducts()):showToast("Error deleting product")}catch(t){console.error("Error:",t),showToast("Error deleting product")}}function fetchOrders(){fetch("/admin/orders").then(t=>t.json()).then(t=>{let i=document.getElementById("ordersTableBody");i.innerHTML="",Array.isArray(t)?(t.forEach(t=>{var e=t.userId?`${t.shippingAddress.street}, ${t.shippingAddress.city}, `+t.shippingAddress.country:"N/A",a=t.userId&&t.userId._id?t.userId.fullName:"N/A",o=t.totalAmount,n=t.status,d=new Date(t.orderedAt).toLocaleDateString(),c=t.shippedAt?new Date(t.shippedAt).toLocaleDateString():"N/A",r=t.deliveredAt?new Date(t.deliveredAt).toLocaleDateString():"N/A";let s="";t.orderItems.forEach(t=>{var e=t.productId?t.productId.name:"Unknown Product",a=t.price,t=t.quantity;s+=`<p>${e} (x${t}) - $${a}</p>`});t=document.createElement("tr");t.innerHTML=`
                        <td>${a}</td>
                        <td>${e}</td>
                        <td>${s}</td>
                        <td>$${o}</td>
    ...
    `).join(""))}async function updateQuantity(t,e){e<1||(await fetch("/cart/update/"+t,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({quantity:e})})).ok&&(loadCart(),updateCartCount())}async function removeItem(t){(await fetch("/cart/delete/"+t,{method:"DELETE"})).ok&&(loadCart(),updateCartCount())}async function clearCart(){(await fetch("/cart/clear",{method:"DELETE"})).ok&&(loadCart(),updateCartCount())}function updateCartTotal(t){t=t.reduce((t,e)=>t+e.productId.price*e.quantity,0);document.getElementById("cartTotal").textContent=t.toFixed(2)}function checkout(){window.location.href="/checkout"}async function loadCheckoutItems(){try{var t=await(await fetch("/cart/items")).json();t.success&&(displayCheckoutItems(t.cartItems),updateTotalAmount(t.cartItems))}catch(t){M.toast({html:"Error loading cart items"})}}function displayCheckoutItems(t){document.getElementById("checkoutItems").innerHTML=t.map(t=>`
        ...
        `).join("")}function displayPagination(a){if(a){var t=document.getElementById("pagination");let e=`
          ...
          `+"</div>",t.innerHTML=e}}function jumpToPage(t){var e=document.getElementById("jumpPage"),e=parseInt(e.value,10);!isNaN(e)&&1<=e&&e<=t?fetchProducts(e):showToast("Please enter a valid page number.")}async function addToCart(t){try{(await(await fetch("/cart/add",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({productId:t})})).json()).success&&(M.toast({html:"Item added to cart"}),updateCartCount())}catch(t){M.toast({html:"Error adding item to cart"})}}checkoutForm?.addEventListener("submit",async t=>{t.preventDefault();t={cardNumber:document.getElementById("cardNumber").value,expiryDate:document.getElementById("expiryDate").value,cvv:document.getElementById("cvv").value,shippingAddress:{street:document.getElementById("street").value,city:document.getElementById("city").value,postalCode:document.getElementById("postalCode").value,country:document.getElementById("country").value}};try{var e=await(await fetch("/orders",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)})).json();e.success&&(io("/order/"+e.order._id).on("statusUpdate",t=>{M.toast({html:"Order status: "+t.status}),updateNotifications(t)}),window.location.href="/homepage")}catch(t){M.toast({html:"Error creating order"})}}),window.addEventListener("load",()=>{let t=document.querySelector(".loader-content");t?(document.body.style.overflow="hidden",setTimeout(()=>{t.style.display="none",document.body.style.overflow="auto"},2e3)):document.body.style.overflow="auto"}),window.addEventListener("DOMContentLoaded",()=>{let t=document.getElementById("authentication");var e=document.getElementById("register"),a=document.getElementById("login"),o=document.getElementById("logout"),n=document.getElementById("manageProduct"),d=document.getElementById("viewOrder"),c=document.getElementById("forgotPassword"),r=document.getElementById("back-to-sign-in"),s=document.getElementById("footer"),i=document.getElementById("toast__success"),l=document.getElementById("toast_error");let u=document.getElementById("userDropdownButton"),m=document.getElementById("userInfoPopup");var p=document.getElementById("searchForm"),l=i||l,y=(document.getElementById("year").textContent=(new Date).getFullYear(),updateCartCount(),null!==l&&(l=l.getAttribute("data-message"),M.toast({html:l,classes:"toast "+(i?"toast__success":"toast__error"),displayLength:4e3})),window.location.pathname);switch(y){case"/":case"/dashboard":s.style.display="none","/dashboard"===y&&(n.addEventListener("click",adminFetchProducts),d.addEventListener("click",fetchOrders),adminModal(["Technology","Clothing","Food","Home & Garden","Sports & Outdoors","Books","Beauty & Health","Toys & Games","Automotive","Pet Supplies"],["Amazon","Walmart","Target","Best Buy","Costco","eBay","Newegg","Home Depot","Macy's","PetSmart"]));break;case"/homepage":fetchProducts();break;case"/cart":loadCart();break;case"/checkout":loadCheckoutItems()}e?.addEventListener("click",()=>{t.classList.add("active")}),a?.addEventListener("click",()=>{t.classList.remove("active")}),o?.addEventListener("click",()=>{fetch("/logout",{method:"POST",credentials:"include"}).then(t=>{t.ok&&(window.location.href="/")}).catch(t=>{console.error("Error logging out:",t)})}),c?.addEventListener("click",t=>{t.preventDefault(),document.getElementById("sign-in-form").style.display="none",document.getElementById("forgot-password-container").style.display="block"}),r?.addEventListener("click",t=>{t.preventDefault(),document.getElementById("sign-in-form").style.display="block",document.getElementById("forgot-password-container").style.display="none"}),p?.addEventListener("submit",t=>{t.preventDefault(),fetchProducts()});l=document.querySelectorAll("select");M.FormSelect.init(l),u&&m&&document.addEventListener("click",t=>{t.target===u?(t.preventDefault(),m.classList.toggle("active")):m.contains(t.target)||m.classList.remove("active")})});let notifications=[];function updateNotifications(t){console.log("Updating notifications with:",t),notifications.unshift(t);var e=document.querySelector(".notification-count"),e=(e&&(e.textContent=notifications.length),document.getElementById("notifications-dropdown"));e&&(e.innerHTML=notifications.map(t=>`
                    <li>
